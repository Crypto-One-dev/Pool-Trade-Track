...
const saveSwapData = async pastEvents => {
  let totalFee = 0
    await pastEvents.forEach(async event => {
      let originDecimals, targetDecimals, originAmount, originUsdAmount, targetAmount, targetPrice, targetUsdAmount, feeInUsd, feeInstance, transactionHash
      const blockNumber = event.blockNumber
      arbOracleInstance.defaultBlock = blockNumber
      let arbOraclePrice = await arbOracleInstance.methods.latestAnswer().call();
      let arbOracleDecimals = await arbOracleInstance.methods.decimals().call();
      
      usdcOracleInstance.defaultBlock = blockNumber
      let usdcOraclePrice = await usdcOracleInstance.methods.latestAnswer().call();
      let usdcOracleDecimals = await usdcOracleInstance.methods.decimals().call();

      let arbPrice = new BigNumber(arbOraclePrice).div(new BigNumber(10).pow(arbOracleDecimals))
      let usdcPrice = new BigNumber(usdcOraclePrice).div(new BigNumber(10).pow(usdcOracleDecimals))
      ...
      csvWriter.writeRecords(tradeData)
        .then(() => {
          ...
          console.log('totalFee In USD: ', totalFee.toFixed(6))
        })
        .catch((error) => {
